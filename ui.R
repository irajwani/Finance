...
shinyUI(
 ...
 column(2, offset = -1,wellPanel( sliderInput("budget","What's your budget?",min=0,max=1000,value=100),
                      ...
                      selectInput("ticker1","Pick a ticker:",c('VGSIX', 'VUSTX', 'VGTSX', 'VFISX', 'VTSMX', 'VFITX', 'VEIEX', 'VIPSX','VUG','VIG','VYM','VOO'),selected = 'VFITX',multiple=FALSE),
                      ...
                      textInput('address2','Type any generic address:',value="Bates College, Lewiston, Maine",placeholder = "Rockstar Games, New York, NY"),
                      selectInput('travelmode','Preffered medium of travel:',c('bicycling','walking','driving'),selected = 'bicycling',multiple = FALSE),
                      ...
                      textInput('publication',label = 'Pick a News publication',value='bloomberg'),
                      textInput('sort',label='Sort By:',value='top')
...
tabPanel("Price and Volume",icon = icon('dollar',lib='font-awesome'),imageOutput("chart1"),imageOutput("chart2")),
tabPanel("Weather",icon=icon("thermometer-half",lib="font-awesome"),htmlOutput("temp"),plotOutput("tempchart")),
tabPanel("I Maps",icon = icon("map", lib="font-awesome"),tableOutput("map"),tableOutput("mapdistance")),
tabPanel('News',icon = icon('wpexplorer',lib='font-awesome'),textOutput('news'))
)
)
)
)
